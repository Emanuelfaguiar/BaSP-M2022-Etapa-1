console.log('05-For');
var names = ['maria', 'pablo', 'juan', 'emanuel', 'alfonso'];
...
for (let i=0; i < names.length; i++){
    upperName[i] = names [i].substring(0,1).toUpperCase();
    lowerName[i] = names[i].substring(1,).toLowerCase();
    upperName[i] += lowerName[i];
    alert(upperName[i]);
}

console.log('Array with Uppercase: ' + upperName);
...
console.log('Array with 10 bucles: ' + empty);